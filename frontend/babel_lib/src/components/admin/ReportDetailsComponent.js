...
const ReportDetailsComponent = ({ report, reportedContent, onResolve, onDelete }) => {
  ...
  const handleBanUser = async () => {
    ...
    try {
      ...
      await addDoc(collection(db, 'notifications'), {
        ...
        type: 'ban',
        message: `Your account has been banned ${selectedBan === 'permanent' ? 'permanently' : `for ${selectedBan} days`}.`,
        createdAt: new Date()
      });

      console.log(`User ${report.reportedUserId} has been banned ${selectedBan === 'permanent' ? 'permanently' : `for ${selectedBan} days`}`);
    } catch (error) {
